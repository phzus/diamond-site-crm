# Task 3 — Row Level Security (RLS): Políticas de Acesso

## Objetivo
Ativar RLS em todas as tabelas e criar as políticas de acesso que garantem que nenhum dado seja acessível sem autenticação e que as permissões por role sejam respeitadas.

## Pré-requisitos
- Task 2 concluída (schema criado)

## Passos de Implementação

### 1. Criar nova migration
```bash
supabase migration new configure_rls_policies
```

### 2. Escrever o SQL de RLS
...
-- Sistema (service role) insere atividades — usuários autenticados também podem
...
-- Sistema (service_role) insere — não bloquear inserção autenticada para server-side
...
No Supabase Dashboard → Authentication → Policies, verificar que todas as tabelas mostram "RLS Enabled" e as políticas listadas.

## Critérios de Conclusão
- [ ] RLS habilitado nas 5 tabelas (`profiles`, `leads`, `lead_notes`, `lead_activities`, `audit_logs`)
- [ ] Função helper `get_user_role()` criada
- [ ] Políticas de `profiles`: admin vê todos, usuário vê o próprio
- [ ] Políticas de `leads`: qualquer autenticado lê/edita, só admin deleta
- [ ] Políticas de `lead_notes`: qualquer autenticado cria/lê, só autor/admin deleta
- [ ] Políticas de `audit_logs`: só admin lê
- [ ] Testado manualmente: usuário sem login não consegue ler dados

## Arquivos Criados/Modificados
- `supabase/migrations/<timestamp>_configure_rls_policies.sql`
