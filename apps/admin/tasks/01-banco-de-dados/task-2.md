# Task 2 — Migrations: Criação do Schema Completo

## Objetivo
Criar e executar todas as migrations que definem o schema do banco de dados: tabelas, tipos, constraints, triggers e índices.

## Pré-requisitos
- Task 1 concluída (CLI configurada e projeto linkado)
...
```
Isso cria o arquivo `supabase/migrations/<timestamp>_create_initial_schema.sql`.

### 2. Escrever o SQL da migration

Preencher o arquivo criado com o seguinte SQL na ordem exata:
...
### 3. Aplicar a migration no banco remoto
```bash
supabase db push
```

### 4. Verificar no dashboard
Acessar `https://supabase.com/dashboard/project/zcwgxmoibuxskhmbtzka/editor` e confirmar que todas as tabelas foram criadas.

## Critérios de Conclusão
- [ ] `supabase db push` executado sem erros
- [ ] Tabelas `profiles`, `leads`, `lead_notes`, `lead_activities`, `audit_logs` existem no banco
- [ ] Trigger `on_auth_user_created` ativo
- [ ] Trigger `set_leads_updated_at` e `set_profiles_updated_at` ativos
- [ ] Índice unique em `leads.email` criado
